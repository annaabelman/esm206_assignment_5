...
### Data Methods: 
The data for the Pacific Giant Salamanders in Mack Creek in Oregon from 1993-2017  were provided by [Aquatic Vertebrate Population Study in Mack Creek, Andrews Experimental Forest](https://doi.org/10.6073/pasta/5de64af9c11579266ef20da2ff32f702).
...
##### B.
**Table 1**: The number of Pacific Giant Salamanders in clear cut and old growth areas found specificially in channels, pools, and side channels. The proportion of each location is shown as percentages.
```{r}
...
kable(pacific_sal_props, col.names = c("Channels", "Pools", "Side Channels"))%>% 
...
ggplot(sal_weight_class, aes(x = unittype, y = weight)) +
  ...
  geom_errorbar(aes(x = unittype,
                ...
                width = 0.1)+
  scale_x_discrete(labels = c("Channels", "Pools", "Side Channels"))+
  labs(x = "Class Classification", y = "Weight (g)", title = "Distribution of Pacific Giant Salamander Weight for Each Classification")+
  theme_minimal()+
  theme(plot.title = element_text(size = 11.5))

```
**Figure 2.** Pacific Giant Salamander's weight distribution (g) (shown in green) and mean weight (black dot) (g) found at the classifications: channels(n = `r num_channel`), pools (n = `r num_pools`), and side channels (n = `r num_side_channel`). The black dot indicates the mean weight, while the bars show the standard error at each all three classification.

(maybe standard error instead of standard deviation) Mention outliers, means, standard deviations. There's a lot more observations in general in channels
